...
function getRandomFilmData() {
	const query = `SELECT film_id, title, description, release_year, length, replacement_cost, rating, special_features FROM film ORDER BY RAND() LIMIT 1`
    const connection = mysql.createConnection(dbconfig);
    connection.connect();
	return new Promise( (resolve,reject) => {
        connection.query(query, (error, data, kentat) =>{
...
async function getFilmFullDataById(film_id){
    try {
        let filmData = ""
        if(film_id == "rand") {
            filmData = await getRandomFilmData()  
        }
        else {
            filmData = await getFilmDataByID(film_id)    
        }
        const filmDataObject = filmData[0]
        filmDataObject["actors"] = await getActorsNameArray(filmDataObject.film_id)
